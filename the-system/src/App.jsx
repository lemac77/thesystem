...
const ProjectsView = ({projects, setProjects, apiKeys={}}) => {
  ...
  const importTasks = async (pid) => {
    ...
    if(anthropicKey) {
      ...
      try {
        const res = await fetch("https://apify-worker.luciettiandrea.workers.dev/anthropic/v1/messages",{
          method:"POST",
          headers:{"Content-Type":"application/json","x-api-key":anthropicKey},
          body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:2000,messages:[{role:"user",content:"Estrai tutte le singole task da questo testo. Ogni task inizia con 'Fase X -' o e separata da asterischi o newline. Restituisci SOLO un JSON array di stringhe. Rimuovi asterischi e link markdown. Nientaltro oltre al JSON array.\n\nTesto:\n"+importText}]})
        });
        const d = await res.json();
    ...
    const raw = importText.replace(/\[([^\]]+)\]\([^)]+\)/g,"$1").split("\n").map(l=>l.trim()).filter(l=>l.length>3);
