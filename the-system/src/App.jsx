...
const CheckinView = ({moodLog,setMoodLog,habits,setHabits,habitLog,setHabitLog,slotDays,slotStart,setSlotDays,setSlotStart}) => {
  ...
  // Focus streak logic
  const focusLog = JSON.parse(localStorage.getItem("ts_focus_log")||"{}");
  const saveFocusLog = (log) => localStorage.setItem("ts_focus_log", JSON.stringify(log));
  const todayFocus = focusLog[today];
  ...
  const handleFocus = (val) => {
    ...
    if(val===false) {
      ...
      setSlotDays(0);
      setSlotStart(today);
      localStorage.setItem("ts_slot_days","0");
      localStorage.setItem("ts_slot_start",today);
    } else if(val===true && todayFocus!==true) {
      // Increment streak
      const newDays = slotDays+1;
      setSlotDays(newDays);
      localStorage.setItem("ts_slot_days",newDays.toString());
    }
    window.location.reload(); // force re-read localStorage
  };
  ...
  const computeStreak = () => {
    ...
    while(s<365){
      ...
      else if(focusLog[k]===false) break;
  ...
  const last14 = Array.from({length:14},(_,i)=>{const d=new Date();d.setDate(d.getDate()-i);const k=d.toISOString().slice(0,10);return {k,short:d.toLocaleDateString("it-IT",{weekday:"short"}),val:focusLog[k]};}).reverse();
      ...
      {subtab==="smoke" && (
        <div>
          ...
          {(() => {
            ...
            const saveSmokeLog = (log) => localStorage.setItem("ts_smoke_log", JSON.stringify(log));
            const todaySmoke = smokeLog[today]||0;
            const setToday = (n) => { saveSmokeLog({...smokeLog,[today]:Math.max(0,n)}); window.dispatchEvent(new Event("smokeupdate")); };

            const last14smoke = Array.from({length:14},(_,i)=>{const d=new Date();d.setDate(d.getDate()-i);const k=d.toISOString().slice(0,10);return {k,short:d.toLocaleDateString("it-IT",{weekday:"short"}),n:smokeLog[k]||0};}).reverse();
