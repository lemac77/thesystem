...
const QuestsView = ({quests, setQuests, moodLog, anthropicKey, onNeedKey}) => {
  ...
  const toggle = async id => {
    ...
    if(!q) return;
    if(!q.done) {
      // Mark done then remove from list after 0.5s (visual feedback)
      const updated = quests.map(x=>x.id===id?{...x,done:true}:x);
      setQuests(updated);
      await pushItem("quests", {...q, done:true});
      setTimeout(()=>setQuests(qs=>qs.filter(x=>x.id!==id)), 500);
    } else {
      // Unmark done
      const updated = quests.map(x=>x.id===id?{...x,done:false}:x);
      setQuests(updated);
      await pushItem("quests", {...q, done:false});
    }
  };
    ...
    <div className="fi">
      <Section title="Daily Quests" action={
        <Row gap={6}>
          ...
          <Btn size="sm" variant="ghost" onClick={async()=>{const d=await DB.pullAll();if(d?.quests)setQuests(d.quests?.filter(q=>!q.done)||[]);}} style={{fontSize:13}}>🔄</Btn>
        ...
        if(data){
          ...
          if(data.quests){
            ...
            if(lr!==t){
              ...
              setQuests(data.quests.filter(q=>!q.done||q.fixed&&!q.done));
            }
          }
        }
        setSbConnected(true);
