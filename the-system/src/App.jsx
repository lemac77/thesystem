...
// ─── LEADS ───────────────────────────────────────────────────────────────────
const LeadsView = ({leads, setLeads}) => {
  const [modal, setModal] = useState(false);
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState({name:"",contact:"",sector:"Altro",stage:"Da contattare",source:"",notes:""});
  const open = l => { setEditing(l?l.id:null); setForm(l||{name:"",contact:"",sector:"Altro",stage:"Da contattare",source:"",notes:""}); setModal(true); };
  ...
  return (
    <div className="fi">
      <Section title="Hunt Pipeline" action={<Btn size="sm" onClick={()=>open(null)}>+ New</Btn>}>
        {leads.length===0 && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:9,color:C.textMuted,padding:"20px 0",textAlign:"center"}}>// Pipeline empty. Begin the hunt.</div>}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {grouped[stage].map(l=>(
                <Card key={l.id}>
                  <Row style={{alignItems:"flex-start",justifyContent:"space-between"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:C.text,fontSize:12,marginBottom:3}}>{l.name}</div>
...
const TasksView = ({tasks, setTasks, anthropicKey, onNeedKey}) => {
  ...
  const parseAndAdd = async () => {
    ...
    setInput("");
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,messages:[{role:"user",content:`Sei l'AI di Studio Brillo (freelance web design, Vicenza). L'utente scrive questo testo libero che contiene uno o più task da fare:
"${raw}"
...
const BrainstormView = ({ideas, setIdeas, anthropicKey, onNeedKey}) => {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,messages:[{role:"user",content:`Sei l'AI di Studio Brillo (freelance web design, Vicenza). L'utente scrive:\n"${raw}"\nRispondi SOLO con JSON:\n{"refined":"idea chiarita in 1-2 frasi","category":"Marketing|Prodotto|Lead Gen|Workflow|Contenuti|Business|Tech","tags":["a","b"],"score":5,"action":"azione concreta da fare ora"}`}]})});
      const d = await res.json();
      const p = JSON.parse((d.content?.[0]?.text||"{}").replace(/```json|```/g,"").trim());
...
// ─── AGENT ────────────────────────────────────────────────────────────────────
const AgentView = ({clients,leads,quests,ideas,workoutLog,dietLog,moodLog,anthropicKey,onNeedKey}) => {
...
Tono: diretto, da mentore, italiano. Conciso e utile.`;
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:ctx,messages:apiMsgs})});
      const d = await res.json();
...
const HunterView = ({onAddToLeads, apiKeys, onNeedKey}) => {
  ...
  const hunt = async () => {
    ...
    try {
      ...
      const r1 = await fetch(`${PROXY}/v2/acts/compass~crawler-google-places/runs?token=${apiKeys.apify}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({searchStringsArray:[`${settore} ${zona} Italia`],maxCrawledPlacesPerSearch:10,language:"it",countryCode:"it"})});
      ...
      const items=await(await fetch(`${PROXY}/v2/actor-runs/${runId}/dataset/items?token=${apiKeys.apify}&limit=10`)).json();
  ...
  const genEmail = async r => {
    if(!apiKeys.anthropic){onNeedKey();return;}
    setLe(p=>({...p,[r.id]:true}));
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,messages:[{role:"user",content:`Sei Andrea di Studio Brillo (studiobrillo.com), freelance web design Vicenza. Email outreach a freddo:\nNome: ${r.name}\nSettore: ${r.category}\nSito: ${r.website||"N/D"}\nRating: ${r.rating||"N/D"} (${r.reviewsCount} rec.)\nMax 120 parole, tono umano, proposta soft su sito. Firma: Andrea - Studio Brillo. Prima riga OGGETTO: [testo]. Niente emoji.`}]})});
      const d=await res.json();
      setResults(p=>p.map(x=>x.id===r.id?{...x,generatedEmail:d.content?.[0]?.text||""}:x));
    } catch(e){setError("Email: "+e.message);}
    finally{setLe(p=>({...p,[r.id]:false}));}
  };
  ...
  return (
    <div className="fi">
      <Section title="Lead Hunter">
        <Card glow style={{marginBottom:14}}>
          <Label>Zona</Label><Input value={zona} onChange={e=>setZona(e.target.value)} placeholder="Vicenza, Bassano..."/>
          <Label>Settore</Label>
          <Select value={settore} onChange={e=>setSettore(e.target.value)}>
            {["Ristorazione","Retail","Fitness","Professionale","Hotel","Artigianato","E-commerce","Luxury"].map(s=><option key={s}>{s}</option>)}
          </Select>
          <Btn style={{width:"100%",justifyContent:"center",padding:"11px",opacity:loading?.6:1}} onClick={hunt} disabled={loading}>
            {loading?`[ ${step||"..."} ]`:"[ 🎯 AVVIA CACCIA ]"}
          </Btn>
          {loading && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:C.textMuted,textAlign:"center",marginTop:7}}>// Apify Google Maps — 30-90s</div>}
        </Card>
        ...
        {results.length>0 && (
          <>
            <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:8,color:C.textDim,letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:10}}>{results.length} targets found</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {results.map(r=>(
                <Card key={r.id} style={{borderColor:r.added?C.success+"55":C.border}}>
                  <div style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:C.text,fontSize:13,marginBottom:6}}>{r.name}</div>
                  {r.address && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:C.textDim,marginBottom:2}}>📍 {r.address}</div>}
                  {r.phone && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:C.textDim,marginBottom:2}}>📞 {r.phone}</div>}
                  {r.website && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:8,color:C.accent,marginBottom:8,wordBreak:"break-all"}}>🌐 {r.website}</div>}
                  <Row gap={7}>
                    {!r.added
                    ...
                    <Btn variant="gold" size="sm" onClick={()=>genEmail(r)} disabled={le[r.id]} style={{opacity:le[r.id]?.6:1}}>
                      {le[r.id]?"...":"✉ Email AI"}
                    </Btn>
                  </Row>
                  {r.generatedEmail && (
                    <div style={{marginTop:10,background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,padding:12}}>
                      <Row style={{justifyContent:"space-between",marginBottom:8}}>
                        <Chip color={C.textDim}>Generated Email</Chip>
                        <button onClick={()=>{navigator.clipboard.writeText(r.generatedEmail).then(()=>{setCopied(p=>({...p,[r.id]:true}));setTimeout(()=>setCopied(p=>({...p,[r.id]:false})),2000);});}} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:3,color:copied[r.id]?C.success:C.textDim,cursor:"pointer",padding:"3px 8px",fontSize:8,fontFamily:"'Rajdhani',sans-serif"}}>
                          {copied[r.id]?"✓ COPIED":"COPY"}
                        </button>
                      </Row>
                      <pre style={{fontFamily:"'Share Tech Mono',monospace",fontSize:9,color:C.text,whiteSpace:"pre-wrap",margin:0,lineHeight:1.6}}>{r.generatedEmail}</pre>
                    </div>
                  )}
                </Card>
              ))}
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100dvh",background:C.bg,color:C.text,maxWidth:580,margin:"0 auto"}}>
      <GS/>
      <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:580,zIndex:50,background:C.bg,borderBottom:`1px solid ${C.border}`}}>
        <Row style={{padding:"10px 14px 0",justifyContent:"space-between"}}>
          <div>
        ...
        <div style={{display:"flex",overflowX:"auto",scrollbarWidth:"none",marginTop:8,borderTop:`1px solid ${C.border}`}}>
          {nav.map(item=>{
      ...
      <div style={{padding:"96px 14px 50px"}}>
        {!loaded ? (
          ...
          <>
            ...
            {tab==="agent" && <AgentView clients={clients} leads={leads} quests={quests} ideas={ideas} workoutLog={workoutLog} dietLog={dietLog} moodLog={moodLog} anthropicKey={apiKeys.anthropic} onNeedKey={openKeys}/>}
            {tab==="hunter" && <HunterView onAddToLeads={addToLeads} apiKeys={apiKeys} onNeedKey={openKeys}/>}
