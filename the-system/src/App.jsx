...
const GoalsView = ({goals, setGoals}) => {
  ...
  const updateCurrent = (id, val) => {
    ...
    if(isNaN(num)) return;
    setGoals(prev=>{
      const updG=prev.map(g=>g.id===id?{...g,current:num,done:num>=(g.target||100)}:g);
          ...
          {active.map(g=>{
            ...
            return (
              <Card key={g.id} glow color={col}>
                ...
                <Row gap={8}>
                  <GoalInput goal={g} onSave={updateCurrent}/>
                  <Btn variant="danger" size="sm" onClick={()=>{setGoals(prev=>prev.filter(x=>x.id!==g.id));deleteItem('goals',g.id);}}>Del</Btn>
                </Row>
              </Card>
            );
          })}
...
// ─── DIARIO / STORICO ─────────────────────────────────────────────────────────
const DiaryView = ({moodLog, setMoodLog, dietLog, setDietLog, smokeLog, setSmokeLog, workoutLog}) => {
      ...
      <Section title="📓 Diario">
        ...
        {wo && (wo.done||wo.type) && (
          <Card>
            ...
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:14,color:wo.done?C.success:C.textDim}}>{wo.done?"💪 Allenato":"—"} {wo.type?`· ${wo.type}`:""}</div>
          </Card>
        )}
      </Section>
    </div>
      ...
      <div style={{padding:"96px clamp(14px,4vw,60px) 50px",maxWidth:860,margin:"0 auto"}}>
        ...
        {loaded && (
          <>
            ...
            {tab==="diary" && <DiaryView moodLog={moodLog} setMoodLog={setMoodLog} dietLog={dietLog} setDietLog={setDietLog} smokeLog={smokeLog} setSmokeLog={setSmokeLog} workoutLog={workoutLog}/>}
