...
// ─── DASHBOARD ───────────────────────────────────────────────────────────────
const Dashboard = ({clients,leads,quests,tasks,workoutLog,dietLog,moodLog,goals,payments,onWeeklyReview,anthropicKey}) => {
        ...
        <Card>
          <Row style={{justifyContent:"space-between",alignItems:"flex-end"}}>
            ...
            <div style={{textAlign:"right"}}>
              ...
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:yearTotal/FORFETTARIO_CAP>0.8?C.danger:C.success}}>
      ...
      <Section title="Oggi">
        <Grid cols={3} gap={8} style={{marginBottom:7}}>
          ...
          <Card style={{textAlign:"center",padding:"12px 8px"}}>
            ...
            {todayDiet?.cheat && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.textMuted,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{todayDiet.cheat}</div>}
          </Card>
          <Card style={{textAlign:"center",padding:"12px 8px"}}>
      ...
      {goals.filter(g=>!g.done).length>0 && (
        <Section title="Goals">
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {goals.filter(g=>!g.done).slice(0,4).map(g=>{
              const pct = g.target ? Math.min(Math.round((g.current/g.target)*100),100) : 0;
              return (
                <Card key={g.id}>
                  <Row style={{justifyContent:"space-between",marginBottom:6}}>
                    <span style={{fontFamily:"'Rajdhani',sans-serif",fontWeight:700,color:C.text,fontSize:16}}>{g.title}</span>
                    <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:pct>=100?C.success:C.gold}}>{pct}%</span>
                  </Row>
                  <ProgressBar pct={pct} color={pct>=100?C.success:C.gold} height={5}/>
                  {g.target && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:C.textDim,marginTop:4}}>{g.current}/{g.target} {g.unit}</div>}
                </Card>
              );
            })}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {quests.map(q=>(
              <Row key={q.id} gap={10}>
                ...
                <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:q.done?C.textMuted:C.text,textDecoration:q.done?"line-through":"none",flex:1,lineHeight:1.4}}>{q.text}</span>
        ...
        <Modal title={editing?"Modifica Cliente":"Nuovo Cliente"} onClose={()=>setModal(false)}>
          ...
          <Grid cols={2} gap={8} style={{marginBottom:12}}>
            ...
            <button onClick={()=>setForm({...form,type:"spot"})} style={{background:form.type==="spot"?`${C.gold}18`:C.bg,border:`2px solid ${form.type==="spot"?C.gold:C.border}`,borderRadius:7,color:form.type==="spot"?C.gold:C.textDim,padding:"10px 8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:16,textTransform:"uppercase",transition:"all .2s",textAlign:"center"}}>
              ⚡ Spot<div style={{fontSize:15,fontWeight:400,marginTop:2,opacity:.7}}>una tantum o a rate</div>
            </button>
          </Grid>
            ...
            <>
              ...
              {form.installments.map((inst,i)=>(
                <Row key={inst.id} gap={6} style={{marginBottom:6,background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,padding:"7px 10px"}}>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.gold}}>{fmtEur(inst.amount)}</div>
                    <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:15,color:C.textDim}}>{inst.dueDate} {inst.label?`— ${inst.label}`:""}</div>
                  </div>
              ...
              <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,padding:10,marginBottom:10}}>
                <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:15,color:C.textDim,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>+ Aggiungi rata</div>
                <Grid cols={2} gap={6}>
    ...
    return (
      <div style={{background:bgCol,border:`1px solid ${borderCol}`,borderRadius:8,padding:"11px 14px",display:"flex",alignItems:"center",gap:10,transition:"all .3s"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:15,color:C.textDim}}>{exp.label} · {exp.dueDate}</div>
        </div>
        <div style={{fontFamily:"'Cinzel',serif",fontSize:16,color:amtCol,fontWeight:700,flexShrink:0}}>{fmtEur(exp.amount)}</div>
        {!received
          ? <button onClick={()=>markReceived(exp)} style={{background:isPast?`${C.orange}18`:C.successDim,border:`1px solid ${isPast?C.orange:C.success}`,borderRadius:5,color:isPast?C.orange:C.success,cursor:"pointer",fontSize:16,padding:"5px 9px",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,letterSpacing:"0.05em",flexShrink:0,textTransform:"uppercase"}}>
              ✓ Ricevuto
            </button>
          : <button onClick={()=>unmark(exp.id)} style={{background:"none",border:`1px solid ${C.success}44`,borderRadius:4,color:C.success,cursor:"pointer",fontSize:15,padding:"4px 8px",fontFamily:"'Rajdhani',sans-serif",flexShrink:0}}>✓ Annulla</button>
        }
      </div>
    );
  };
  ...
  return (
    <div className="fi">
      ...
      <Section title="Finanze" action={<Btn size="sm" variant="gold" onClick={()=>setManualModal(true)}>+ Extra</Btn>}>
        <Card glow color={forfPct>80?C.danger:C.gold} style={{marginBottom:14}}>
          <Grid cols={3} gap={10} style={{marginBottom:10}}>
            ...
            <div>
              ...
              <div style={{fontFamily:"'Cinzel',serif",fontSize:17,color:C.accent,lineHeight:1}}>{fmtEur(monthReceivedTotal)}<span style={{fontSize:"clamp(9px,0.9vw,13px)",color:C.textDim}}>/{fmtEur(monthExpectedTotal)}</span></div>
            </div>
            <div>
              <Label>Forfettario</Label>
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:forfPct>80?C.danger:forfPct>60?C.gold:C.success}}>{forfPct}%</div>
            </div>
          </Grid>
          <ProgressBar pct={forfPct} color={forfPct>80?C.danger:C.gold} height={5}/>
          <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.textMuted,marginTop:5}}>{fmtEur(yearTotal)} / {fmtEur(FORFETTARIO_CAP)} — rimangono {fmtEur(FORFETTARIO_CAP-yearTotal)}</div>
        </Card>
        ...
        {overdue.filter(e=>!isReceived(e.id)).length>0 && (
          <div style={{marginBottom:20}}>
            <Row gap={8} style={{marginBottom:10}}>
              ...
              <div style={{fontFamily:"'Cinzel',serif",fontSize:16,color:C.orange,letterSpacing:"0.12em",textTransform:"uppercase"}}>In sospeso — {fmtEur(overdueTotal)}</div>
            </Row>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
        ...
        {/* Questo mese */}
        <div style={{marginBottom:20}}>
          <div style={{fontFamily:"'Cinzel',serif",fontSize:16,color:C.accent,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:10}}>
            {monthName}
          </div>
          {thisMonthExp.length===0
            ? <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.textMuted,padding:"10px 0"}}>// Nessun pagamento atteso questo mese.</div>
        ...
        {future.length>0 && (
          <div style={{marginBottom:20}}>
            <div style={{fontFamily:"'Cinzel',serif",fontSize:16,color:C.textDim,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:10}}>Prossimi mesi</div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {future.slice(0,6).map(exp=>(
                <div key={exp.id} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:7,padding:"9px 14px",display:"flex",alignItems:"center",gap:10,opacity:.7}}>
                  <div style={{flex:1}}>
                    <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:15,color:C.textDim,fontWeight:600}}>{exp.clientName}</div>
                    <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:15,color:C.textMuted}}>{exp.label} · {exp.dueDate}</div>
                  </div>
                  <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.textDim,flexShrink:0}}>{fmtEur(exp.amount)}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {(() => {
          ...
          return (
            <div>
              <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:"clamp(9px,0.9vw,13px)",color:C.textDim,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:8}}>Storico incassato {thisYear}</div>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {[...received].sort((a,b)=>b.date.localeCompare(a.date)).map(p=>(
                  <Row key={p.id} style={{background:C.successDim,border:`1px solid ${C.success}33`,borderRadius:6,padding:"9px 12px",justifyContent:"space-between"}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:15,color:C.text,fontWeight:600}}>
                        {clients.find(c=>c.id===p.clientId)?.name||"Entrata"}
                      </div>
                      {p.note && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:15,color:C.textDim}}>{p.note}</div>}
                      <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:C.textMuted}}>{p.date}</div>
                    </div>
                    <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:15,color:C.success,flexShrink:0,marginRight:6}}>{fmtEur(p.amount)}</div>
...
const CheckinView = ({moodLog,setMoodLog,habits,setHabits,habitLog,setHabitLog,slotDays,slotStart,setSlotDays,setSlotStart}) => {
  const [smokeLog, setSmokeLog] = useState(()=>JSON.parse(localStorage.getItem("ts_smoke_log")||"{}"));
  const [subtab, setSubtab] = useState("mood");
  const today = todayStr();



  const [focusLogState, setFocusLogState] = useState(()=>JSON.parse(localStorage.getItem("ts_focus_log")||"{}"));
  const handleFocus = (val) => {
    const newLog = {...focusLogState,[today]:val};
    localStorage.setItem("ts_focus_log", JSON.stringify(newLog));
    setFocusLogState(newLog);
    if(val===false) {
      setSlotDays(0);
      setSlotStart(today);
    } else if(val===true && focusLogState[today]!==true) {
      setSlotDays(s=>s+1);
    }
  };

  const todayFocus = focusLogState[today];

  // Recompute streak from focusLog
  const computeStreak = () => {
    let s=0; const d=new Date();
    while(s<365){
      const k=d.toISOString().slice(0,10);
      if(focusLogState[k]===true){s++;d.setDate(d.getDate()-1);}
      else if(focusLogState[k]===false) break;
      else {d.setDate(d.getDate()-1);if(s>0)break;}
    }
    return s;
  };
  const streak = computeStreak();

  const last14 = Array.from({length:14},(_,i)=>{const d=new Date();d.setDate(d.getDate()-i);const k=d.toISOString().slice(0,10);return {k,short:d.toLocaleDateString("it-IT",{weekday:"short"}),val:focusLogState[k]};}).reverse();

  return (
    <div className="fi">
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {[["mood","Mood"],["habits","Abitudini"],["focus","Streak"],["smoke","🚬"]].map(([k,l])=>(
          <button key={k} onClick={()=>setSubtab(k)} style={{flex:1,minWidth:"23%",background:subtab===k?C.accentDim:"transparent",border:`1px solid ${subtab===k?C.accent:C.border}`,borderRadius:5,color:subtab===k?C.accent:C.textDim,padding:"8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:14,letterSpacing:"0.08em",textTransform:"uppercase",transition:"all .2s"}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {subtab==="focus" && (
        <div>
          ...
          <Card glow color={streak>7?C.success:streak>3?C.gold:C.accent} style={{marginBottom:14,textAlign:"center",padding:"24px 16px"}}>
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.textDim,letterSpacing:"0.2em",marginBottom:8}}>STREAK</div>
            ...
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.textDim,marginTop:8}}>giorni</div>
          </Card>
          ...
          <Card style={{marginBottom:14}}>
            ...
            <Grid cols={2} gap={8} style={{marginTop:8}}>
              <button onClick={()=>todayFocus!==true&&handleFocus(true)} style={{background:todayFocus===true?C.successDim:"transparent",border:`2px solid ${todayFocus===true?C.success:C.border}`,borderRadius:7,color:todayFocus===true?C.success:C.textDim,padding:"14px 8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:15,textTransform:"uppercase",transition:"all .2s",textAlign:"center"}}>
                <div style={{fontSize:28,marginBottom:4}}>✅</div>NO
              </button>
              <button onClick={()=>todayFocus!==false&&handleFocus(false)} style={{background:todayFocus===false?C.dangerDim:"transparent",border:`2px solid ${todayFocus===false?C.danger:C.border}`,borderRadius:7,color:todayFocus===false?C.danger:C.textDim,padding:"14px 8px",cursor:"pointer",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:15,textTransform:"uppercase",transition:"all .2s",textAlign:"center"}}>
                <div style={{fontSize:28,marginBottom:4}}>❌</div>SÌ
              </button>
            </Grid>
            {todayFocus===false && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.danger,marginTop:10,textAlign:"center"}}>// streak azzerata. domani si ricomincia.</div>}
            {todayFocus===true && <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.success,marginTop:10,textAlign:"center"}}>// +1 giorno. tieni duro.</div>}
          </Card>
          ...
          <Card>
            ...
            <div style={{display:"flex",gap:4,marginTop:10}}>
              {last14.map(({k,short,val})=>(
                <div key={k} style={{flex:1,textAlign:"center"}}>
                  <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:C.textMuted,marginBottom:4}}>{short}</div>
                  <div style={{aspectRatio:"1",borderRadius:4,background:val===true?C.successDim:val===false?C.dangerDim:C.bg,border:`1px solid ${val===true?C.success:val===false?C.danger:C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>
                    {val===true?"✓":val===false?"×":""}
                  </div>
                </div>
              ))}
      ...
      {subtab==="smoke" && (
        <div>
          {/* Today counter */}
          {(()=>{
            const smokeLogState = smokeLog; const setSmokeLogState = setSmokeLog;
            const todaySmoke = smokeLogState[today]||0;
            const setToday = (n) => {
              const newLog = {...smokeLogState,[today]:Math.max(0,n)};
              localStorage.setItem("ts_smoke_log", JSON.stringify(newLog));
              setSmokeLogState(newLog);
            };
            const last14smoke = Array.from({length:14},(_,i)=>{const d=new Date();d.setDate(d.getDate()-i);const k=d.toISOString().slice(0,10);return {k,short:d.toLocaleDateString("it-IT",{weekday:"short"}),n:smokeLogState[k]||0};}).reverse();
            const avg7 = last14smoke.slice(7).reduce((s,x)=>s+x.n,0)/7;
            const avg7prev = last14smoke.slice(0,7).reduce((s,x)=>s+x.n,0)/7;
            const maxDay = Math.max(...last14smoke.map(x=>x.n),1);
            return (
              <>
                <Card glow style={{marginBottom:14,textAlign:"center",padding:"20px 16px"}}>
                  <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.textDim,letterSpacing:"0.15em",marginBottom:8}}>OGGI</div>
                  ...
                  <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.textDim,marginTop:6}}>sigarette</div>
                  <Row gap={12} style={{justifyContent:"center",marginTop:14}}>
                    <button onClick={()=>setToday(todaySmoke-1)} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,cursor:"pointer",padding:"8px 20px",fontFamily:"'Cinzel',serif",fontSize:20,fontWeight:700}}>−</button>
                    <button onClick={()=>setToday(todaySmoke+1)} style={{background:C.accentDim,border:`1px solid ${C.accent}`,borderRadius:6,color:C.accent,cursor:"pointer",padding:"8px 20px",fontFamily:"'Cinzel',serif",fontSize:20,fontWeight:700}}>+</button>
                  </Row>
                </Card>
                ...
                <Card style={{marginBottom:14}}>
                  <Row style={{justifyContent:"space-between",marginBottom:10}}>
                    <div>
                      <Label>Media questa settimana</Label>
                      <div style={{fontFamily:"'Cinzel',serif",fontSize:22,color:avg7<avg7prev?C.success:avg7>avg7prev?C.danger:C.gold,fontWeight:700}}>{avg7.toFixed(1)}<span style={{fontSize:13,color:C.textDim}}>/giorno</span></div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <Label>Settimana scorsa</Label>
                      <div style={{fontFamily:"'Cinzel',serif",fontSize:22,color:C.textDim,fontWeight:700}}>{avg7prev.toFixed(1)}<span style={{fontSize:13,color:C.textMuted}}>/giorno</span></div>
                    </div>
                  </Row>
                  {avg7<avg7prev
                    ? <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.success}}>↓ {(avg7prev-avg7).toFixed(1)} meno rispetto alla scorsa settimana</div>
                    : avg7>avg7prev
                    ? <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.danger}}>↑ {(avg7-avg7prev).toFixed(1)} in più rispetto alla scorsa settimana</div>
                    : <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:C.textDim}}>= stabile rispetto alla scorsa settimana</div>
                  }
                </Card>
                  ...
                  <div style={{display:"flex",gap:3,alignItems:"flex-end",height:60,marginTop:10}}>
                    {last14smoke.map(({k,short,n})=>(
                      <div key={k} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                        ...
                        <div style={{width:"100%",background:n>15?C.danger:n>8?C.orange:n>0?C.gold:C.border,borderRadius:"2px 2px 0 0",height:`${Math.max((n/maxDay)*50,n>0?4:2)}px`,transition:"height .3s"}}/>
    ...
    <div className="app-shell" style={{minHeight:"100dvh",background:C.bg,color:C.text,fontSize:window.innerWidth>1400?"20px":window.innerWidth>1100?"18px":window.innerWidth>768?"16px":"14px"}}>
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,zIndex:50,background:C.bg,borderBottom:`1px solid ${C.border}`}}>
        <Row style={{padding:"10px clamp(14px,4vw,60px) 0",justifyContent:"space-between"}}>
          ...
          <Row gap={8}>
            ...
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:16,color:sbConnected?C.success:apiKeys.anthropic?C.accent:C.danger}}>
      ...
      <div style={{padding:"96px clamp(14px,4vw,60px) 50px",maxWidth:860,margin:"0 auto"}}>
        {!loaded ? (
          ...
          <>
            {tab==="dashboard" && <Dashboard clients={clients} leads={leads} quests={quests} tasks={tasks} workoutLog={workoutLog} dietLog={dietLog} moodLog={moodLog} goals={goals} payments={payments} onWeeklyReview={generateWeeklyReport} anthropicKey={apiKeys.anthropic}/>}
            ...
            {tab==="checkin" && <CheckinView moodLog={moodLog} setMoodLog={setMoodLog} habits={habits} setHabits={setHabits} habitLog={habitLog} setHabitLog={setHabitLog} slotDays={slotDays} slotStart={slotStart} setSlotDays={setSlotDays} setSlotStart={setSlotStart}/>}
