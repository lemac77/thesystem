...
{"analisi":"2-3 frasi su come sta andando","rimuovi":["testo esatto quest da togliere"],"aggiungi_fisse":["nuove quest fisse"],"aggiungi_variabili":["nuove quest per oggi"],"modifica":[{"da":"testo originale","a":"testo nuovo"}]}`;
      const res = await fetch("https://apify-worker.luciettiandrea.workers.dev/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,messages:[{role:"user",content:prompt}]})});
...
Tono: diretto, da mentore, italiano. Conciso e utile.`;
    try {
      ...
      const res = await fetch("https://apify-worker.luciettiandrea.workers.dev/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:ctx,messages:apiMsgs})});
      const d = await res.json();
...
export default function App() {
  ...
  return (
    <div className="app-shell" style={{minHeight:"100dvh",background:C.bg,color:C.text,maxWidth:"100%",margin:"0 auto"}}>
      <GS/>
      <div style={{position:"fixed",top:0,left:0,width:"100%",zIndex:50,background:C.bg,borderBottom:`1px solid ${C.border}`}}>
      ...
      <div style={{padding:"96px clamp(14px, 4vw, 60px) 50px",maxWidth:960,margin:"0 auto"}}>
        {!loaded ? (
