...
{"quests":["testo quest 1","testo quest 2","testo quest 3"]}`;
  ...
  // Periodic pull for frequently changing data (every 60s)
  useEffect(()=>{
    if(!getSB()) return;
    const interval = setInterval(async()=>{
      const token = await sbAuth.getValidToken();
      if(!token) return;
      const [smokeSetting, focusSetting, moodSetting] = await Promise.all([
        DB.getSetting("smoke_log",null),
        DB.getSetting("focus_log",null),
        DB.getSetting("mood_log",null),
      ]);
      if(smokeSetting) setSmokeLog(smokeSetting);
      if(focusSetting) setFocusLog(focusSetting);
      if(moodSetting) setMoodLog(moodSetting);
    }, 60000);
