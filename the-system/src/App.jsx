...
{"analisi":"2-3 frasi su come sta andando","rimuovi":["testo esatto quest da togliere"],"aggiungi_fisse":["nuove quest fisse"],"aggiungi_variabili":["nuove quest per oggi"],"modifica":[{"da":"testo originale","a":"testo nuovo"}]}`;
      const res = await fetch("https://apify-worker.luciettiandrea.workers.dev/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":anthropicKey},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,messages:[{role:"user",content:prompt}]})});
...
const LeadsView = ({leads, setLeads, apiKeys={}, onNeedKey=()=>{}}) => {
  ...
  return (
    <div className="fi">
      ...
      {subtab==="pipeline" && <Section title="Pipeline" action={<Btn size="sm" onClick={()=>open(null)}>+ New</Btn>}>
...
Tono: diretto, da mentore, italiano. Conciso e utile.`;
    try {
      ...
      const res = await fetch("https://apify-worker.luciettiandrea.workers.dev/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":anthropicKey},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:ctx,messages:apiMsgs})});
      const d = await res.json();
