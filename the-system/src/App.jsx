      ...
      <Section title="Daily Quests" action={
        <Row gap={6}>
          ...
          <Btn size="sm" variant="ghost" onClick={async()=>{
            // Push current state for quests first
            for(const q of quests) await pushItem("quests", q);
            // Then pull
            const d=await DB.pullAll();
            if(d?.quests)setQuests(d.quests);
          }} style={{fontSize:13}}>🔄</Btn>
    ...
    <div className="fi">
      <Section title="Task" action={
        <Row gap={6}>
          ...
          <Btn size="sm" variant="ghost" onClick={async()=>{
            for(const t of tasks) await pushItem("tasks", t);
            const d=await DB.pullAll();
...
{"quests":["testo quest 1","testo quest 2","testo quest 3"]}`;
  ...
  // No more auto-push - all changes go through pushItem immediately
