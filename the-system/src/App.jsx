...
const QuestsView = ({quests, setQuests, moodLog, anthropicKey, onNeedKey}) => {
  ...
  const toggle = async id => {
    ...
    if(!q) return;
    const updated = quests.map(x=>x.id===id?{...x,done:!x.done}:x);
    setQuests(updated);
    await pushItem("quests", {...q, done:!q.done});
  };
...
{"analisi":"2-3 frasi su come sta andando","rimuovi":["testo esatto quest da togliere"],"aggiungi_fisse":["nuove quest fisse"],"aggiungi_variabili":["nuove quest per oggi"],"modifica":[{"da":"testo originale","a":"testo nuovo"}]}`;
  ...
  const QRow = ({q, canDel}) => (
    <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:C.bg,border:`1px solid ${q.done?C.success+"55":C.border}`,borderRadius:6,transition:"border-color .2s"}}>
      ...
      {canDel && <button onClick={()=>{setQuests(quests.filter(x=>x.id!==q.id));deleteItem('quests',q.id);}} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:15,lineHeight:1,padding:"0 2px"}}>×</button>}
    </div>
  );
...
const ClientsView = ({clients, setClients}) => {
  ...
  const saveC = () => {
    ...
    let savedClient;
    if(editing){ savedClient={...clients.find(c=>c.id===editing),...client}; setClients(clients.map(c=>c.id===editing?savedClient:c)); }
    else{ savedClient={id:uid(),...client,created_at:todayStr()}; setClients([...clients,savedClient]); }
            ...
            <Card key={c.id}>
              <Row style={{alignItems:"flex-start",justifyContent:"space-between"}}>
                ...
                <Row gap={5} style={{flexShrink:0}}>
                  ...
                  <Btn variant="danger" size="sm" onClick={()=>{setClients(clients.filter(x=>x.id!==c.id));deleteItem('clients',c.id);}}>Del</Btn>
                </Row>
              </Row>
            </Card>
          ))}
...
const FinanceView = ({clients, payments, setPayments}) => {
  ...
  const markPaid = (c) => {
    ...
    const np={id:uid(),clientId:c.id,amount:parseFloat(c.monthlyFee)||0,date:todayISO,note:`canone-${thisMonth}`,expectedId:`rec-${c.id}-${thisMonth}`};setPayments([...payments,np]);pushItem('payments',np);
  };
  const unmarkPaid = (clientId) => { const toDelRec=payments.filter(p=>p.clientId===clientId && p.note===`canone-${thisMonth}`); setPayments(payments.filter(p=>!(p.clientId===clientId && p.note===`canone-${thisMonth}`))); toDelRec.forEach(p=>deleteItem('payments',p.id)); };
  ...
  const markInstPaid = (c, inst) => {
    ...
    const ni2={id:uid(),clientId:c.id,amount:parseFloat(inst.amount)||0,date:todayISO,note:inst.label||'Rata',expectedId:`exp-${c.id}-${inst.id}`};setPayments([...payments,ni2]);pushItem('payments',ni2);
  };
  ...
  const saveManual = () => {
    ...
    const nm={id:uid(),...manualForm};setPayments([...payments,nm]);pushItem('payments',nm);
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {[...yearPayments].sort((a,b)=>b.date?.localeCompare(a.date||"")).map(p=>(
                <Row key={p.id} style={{background:`${C.success}08`,border:`1px solid ${C.success}22`,borderRadius:6,padding:"9px 12px",justifyContent:"space-between"}}>
                  ...
                  <button onClick={()=>{setPayments(payments.filter(x=>x.id!==p.id));deleteItem('payments',p.id);}} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:15}}>×</button>
                </Row>
              ))}
            </div>
          </div>
                  ...
                  <Card key={l.id}>
                    <Row style={{alignItems:"flex-start",justifyContent:"space-between"}}>
                      ...
                      <Row gap={5} style={{flexShrink:0}}>
                        ...
                        <Btn variant="danger" size="sm" onClick={()=>{setLeads(leads.filter(x=>x.id!==l.id));deleteItem('leads',l.id);}}>Del</Btn>
                      </Row>
                    </Row>
                  </Card>
                ))}
...
const GoalsView = ({goals, setGoals}) => {
  ...
  const save = () => {
    ...
    const ng={id:uid(),...form,current:parseFloat(form.current)||0,target:parseFloat(form.target)||100,done:false,created_at:todayStr()};
    setGoals([...goals,ng]);
    pushItem("goals",ng);
    setModal(false);
    setForm({title:"",target:"",unit:"",current:"",deadline:"",category:"Business"});
  };
  const updateCurrent = (id, val) => { const updG=goals.map(g=>g.id===id?{...g,current:parseFloat(val)||0,done:(parseFloat(val)||0)>=(g.target||100)}:g); setGoals(updG); pushItem('goals',updG.find(g=>g.id===id)); };
            ...
            return (
              <Card key={g.id} glow color={col}>
                ...
                <Row gap={8}>
                  ...
                  <Btn variant="danger" size="sm" onClick={()=>{setGoals(goals.filter(x=>x.id!==g.id));deleteItem('goals',g.id);}}>Del</Btn>
                </Row>
              </Card>
            );
          })}
                ...
                <Card key={g.id} style={{borderColor:C.success+"33"}}>
                  <Row style={{justifyContent:"space-between"}}>
                    ...
                    <Row gap={6}>
                      ...
                      <button onClick={()=>{setGoals(goals.filter(x=>x.id!==g.id));deleteItem('goals',g.id);}} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:14}}>×</button>
                    </Row>
                  </Row>
                </Card>
              ))}
            ...
            <Card key={n.id} style={{borderColor:pinned[n.id]?C.gold+"66":C.border}}>
              <Row style={{justifyContent:"space-between",marginBottom:6}}>
                ...
                <Row gap={6}>
                  ...
                  <button onClick={()=>{setNotes(notes.filter(x=>x.id!==n.id));deleteItem('notes',n.id);}} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:15,lineHeight:1}}>×</button>
                </Row>
              </Row>
