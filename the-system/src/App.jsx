...
const DB = {
  upsert: async (table, rows) => {
    ...
    await sbReqAuth(table+"?on_conflict=id", {method:"POST", prefer:"return=minimal", headers:{"Prefer":"resolution=merge-duplicates,return=minimal"}, body:JSON.stringify(rows.map(r=>({...r,user_id:(()=>{try{return JSON.parse(localStorage.getItem("ts_auth_user")||"{}").id||"andrea";}catch{return "andrea";}})()} )))});
  },
  setSetting: async (key, value) => {
    await sbReqAuth("settings", {method:"POST", headers:{"Prefer":"resolution=merge-duplicates,return=minimal"}, body:JSON.stringify({key, value:JSON.stringify(value), user_id:(()=>{try{return JSON.parse(localStorage.getItem("ts_auth_user")||"{}").id||"andrea";}catch{return "andrea";}})()} )});
  },
  getSetting: async (key, def) => {
    const rows = await sbReqAuth(`settings?key=eq.${encodeURIComponent(key)}&user_id=eq.andrea&select=value`);
    if(!rows?.length) return def;
    try { return JSON.parse(rows[0].value); } catch { return def; }
  },
  getAll: async (table, def=[]) => {
    const rows = await sbReqAuth(`${table}?user_id=eq.andrea`);
    return rows||def;
  },
  pushAll: async (s) => {
    if(!getSB()) return;
  ...
  pullAll: async () => {
    ...
    try {
      const [clients,leads,quests,tasks,ideas,goals,notes,payments,
        ...
        slotDays,slotStart,lastReset,apiKeys,smokeLog,focusLog] = await Promise.all([
      ...
      return {clients,leads,quests,tasks,ideas,goals,notes,payments,
        workoutLog,dietLog,moodLog,weightLog,habitLog,habits,
        slotDays,slotStart,lastReset,apiKeys,smokeLog,focusLog};
    } catch { return null; }
  },
};
...
const sbAuth = {
  signIn: async (email, password) => {
    ...
    try {
      ...
      let d;
      try { d = JSON.parse(text); } catch { return {error:"Risposta non valida dal server"}; }
      if(d.access_token) {
...
const Dashboard = ({clients,leads,quests,tasks,workoutLog,dietLog,moodLog,goals,payments,onWeeklyReview,anthropicKey}) => {
  ...
  const [todayCigs, setTodayCigs] = useState(()=>{try{return JSON.parse(localStorage.getItem("ts_smoke_log")||"{}")[today]||0;}catch{return 0;}});
  useEffect(()=>{
    const handler = ()=>{try{setTodayCigs(JSON.parse(localStorage.getItem("ts_smoke_log")||"{}")[today]||0);}catch{}};
    window.addEventListener("smokechange",handler);
    return ()=>window.removeEventListener("smokechange",handler);
  },[today]);
  ...
  return (
    <div className="fi">
      ...
      {/* Oggi */}
      <Section title="Oggi">
        <Grid cols={4} gap={8} style={{marginBottom:7}}>
          ...
          <Card style={{textAlign:"center",padding:"10px 6px"}}>
            <div style={{fontFamily:"'Cinzel',serif",fontSize:26,fontWeight:900,color:todayCigs>15?C.danger:todayCigs>8?C.orange:C.gold,lineHeight:1,marginBottom:2}}>{todayCigs}</div>
            <div style={{fontSize:16,marginBottom:6}}>🚬</div>
            <Row gap={4} style={{justifyContent:"center"}}>
              <button onClick={()=>{const l=JSON.parse(localStorage.getItem("ts_smoke_log")||"{}");l[today]=Math.max(0,(l[today]||0)-1);localStorage.setItem("ts_smoke_log",JSON.stringify(l));window.dispatchEvent(new CustomEvent("smokechange"));setTodayCigs(Math.max(0,todayCigs-1));}} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:3,color:C.text,cursor:"pointer",padding:"2px 8px",fontSize:14,fontWeight:700,lineHeight:1}}>−</button>
              <button onClick={()=>{const l=JSON.parse(localStorage.getItem("ts_smoke_log")||"{}");l[today]=(l[today]||0)+1;localStorage.setItem("ts_smoke_log",JSON.stringify(l));window.dispatchEvent(new CustomEvent("smokechange"));setTodayCigs(todayCigs+1);}} style={{background:C.accentDim,border:`1px solid ${C.accent}`,borderRadius:3,color:C.accent,cursor:"pointer",padding:"2px 8px",fontSize:14,fontWeight:700,lineHeight:1}}>+</button>
            </Row>
          </Card>
        </Grid>
      </Section>
...
const CheckinView = ({moodLog,setMoodLog,habits,setHabits,habitLog,setHabitLog,slotDays,setSlotDays,setSlotStart}) => {
  const [subtab, setSubtab] = useState("mood");
  const today = todayStr();

  // Focus streak state
  const [focusLog, setFocusLog] = useState(()=>{try{return JSON.parse(localStorage.getItem("ts_focus_log")||"{}");}catch{return {};}});
  ...
  // Smoke counter state
  const [smokeLog, setSmokeLog] = useState(()=>{try{return JSON.parse(localStorage.getItem("ts_smoke_log")||"{}");}catch{return {};}});
  const todaySmoke = smokeLog[today]||0;

  const updateSmoke = (n) => {
    const val = Math.max(0,n);
    const newLog = {...smokeLog,[today]:val};
...
{"quests":["testo quest 1","testo quest 2","testo quest 3"]}`;
  ...
  stateRef.current = {clients,leads,quests,tasks,ideas,goals,notes,payments,workoutLog,dietLog,moodLog,weightLog,habitLog,habits,slotDays,slotStart,apiKeys,smokeLog:LS.get('ts_smoke_log',{}),focusLog:LS.get('ts_focus_log',{})};
  useEffect(()=>{
    if(!loaded||!getSB()) return;
    const t = setTimeout(()=>DB.pushAll(stateRef.current),1500);
    return ()=>clearTimeout(t);
  },[clients,leads,quests,tasks,ideas,goals,notes,payments,workoutLog,dietLog,moodLog,weightLog,habitLog,habits,slotDays,slotStart,loaded]); // smoke/focus sync via stateRef reading localStorage
